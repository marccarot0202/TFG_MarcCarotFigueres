 ...
 */
export function isSafeDynamicKey(key) {
...
export function isSafeChainId(chainId) {
    ...
    const decimalChainId = Number.parseInt(chainId, isStrictHexString(chainId) ? 16 : 10);
 ...
 */
export function fractionBN(targetBN, numerator, denominator) {
 ...
 */
export function getBuyURL(networkCode = '1', address, amount = 5) {
    switch (networkCode) {
        case '1':
...
export function toHex(value) {
    ...
    const hexString = BN.isBN(value) || typeof value === 'bigint'
        ? value.toString(16)
 ...
 */
export function isSmartContractCode(code) {
    /* istanbul ignore if */
    if (!code) {
        return false;
    }
 ...
 */
export async function fetchWithErrorHandling({ url, options, timeout, errorCodesToCatch, }) {
    let result;
    try {
        if (timeout) {
            result = Promise.race([
 ...
 */
export async function timeoutFetch(url, options, timeout = 500) {
    return Promise.race([
 ...
 * @returns Promise resolving the request.
 */
export function query(ethQuery, method, 
 ...
 */
export const convertHexToDecimal = (value = '0x0') => {
