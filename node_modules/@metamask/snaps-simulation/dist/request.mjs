 ...
 */
export function handleRequest({ snapId, store, executionService, handler, controllerMessenger, simulationOptions, runSaga, request: { id = nanoid(), origin = 'https://metamask.io', ...options }, }) {
    ...
    promise.getInterface = async () => {
        const sagaPromise = runSaga(getInterface, runSaga, snapId, controllerMessenger, simulationOptions).toPromise();
...
export async function getInterfaceFromResult(result, snapId, controllerMessenger) {
    ...
    if (isPlainObject(result) && hasProperty(result, 'content')) {
        ...
        const id = controllerMessenger.call('SnapInterfaceController:createInterface', snapId, result.content);
        return id;
    }
    return undefined;
}
 ...
 */
export async function getInterfaceApi(result, snapId, controllerMessenger, options, contentId) {
