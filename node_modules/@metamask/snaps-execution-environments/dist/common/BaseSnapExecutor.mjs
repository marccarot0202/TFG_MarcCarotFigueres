...
export class BaseSnapExecutor {
    ...
    async onCommandRequest(message) {
        if (!isJsonRpcRequest(message)) {
            if (hasProperty(message, 'id') &&
                ...
                logInfo('Command stream received a non-JSON-RPC request, and was unable to respond.');
            }
            return;
        }
    ...
    async #notify(notification) {
        if (!isValidResponse(notification)) {
            throw rpcErrors.internal('JSON-RPC notifications must be JSON serializable objects smaller than 64 MB.');
        }
        await this.#write({
            ...notification,
            jsonrpc: '2.0',
    ...
    async #respond(id, response) {
        if (!isValidResponse(response)) {
            ...
            await this.#write({
                error: serializeError(rpcErrors.internal('JSON-RPC responses must be JSON serializable objects smaller than 64 MB.')),
                id,
                jsonrpc: '2.0',
            });
            return;
    ...
    async startSnap(snapId, sourceCode, _endowments) {
        ...
        const provider = new SnapProvider(multiplex.createStream('metamask-provider'), {
