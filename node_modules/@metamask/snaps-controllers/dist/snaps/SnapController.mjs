...
export class SnapController extends BaseController {
    ...
    #handlePreinstalledSnaps(preinstalledSnaps) {
        ...
        for (const snap of Object.values(this.state.snaps).filter(({ preinstalled }) => preinstalled)) {
            const processedPermissions = processSnapPermissions(snap.manifest.initialPermissions);
            this.#validateSnapPermissions(processedPermissions);
            ...
            if (isNonEmptyArray(Object.keys(newPermissions)) ||
                ...
                const { proposedName } = getLocalizedSnapManifest(snap.manifest, 'en', snap.localizationFiles ?? []);
                ...
                logWarning(`The permissions for "${snap.id}" were out of sync and have been automatically restored. If you see this message, please file a bug report.`);
    ...
    async #ensureCanUsePlatform() {
        ...
        assert(isReady, 'The Snaps platform requires basic functionality to be used. Enable basic functionality in the settings to use the Snaps platform.');
    }
    ...
    async getSnapFile(snapId, path, encoding = AuxiliaryFileEncoding.Base64) {
        ...
        assert(encoded.length < MAX_FILE_SIZE, `Failed to encode static file to "${encoding}": Static files must be less than 64 MB when encoded.`);
        return encoded;
    }
    /**
    ...
    async #add(args) {
        ...
        if (!runtime.installPromise) {
            ...
            runtime.installPromise = (async () => {
                ...
                const preinstalledArgs = this.#featureFlags.forcePreinstalledSnaps && isLocalSnapId(snapId)
                    ? {
                        preinstalled: true,
    ...
    async #transformSnapRpcResponse(snapId, handlerType, request, result) {
        switch (handlerType) {
            ...
            case HandlerType.OnAssetsMarketData:
                // We can cast since the request and result have already been validated.
                return this.#transformOnAssetsMarketDataResult(request, result);
            default:
                return result;
        }
    }
     ...
     * @returns The transformed result.
     */
    #transformOnAssetsMarketDataResult({ params: requestedParams }, { marketData }) {
        const { assets: requestedAssets } = requestedParams;
        const filteredMarketData = requestedAssets.reduce((accumulator, assets) => {
    ...
    #callLifecycleHooks(origin, handler) {
        ...
        for (const { id } of snaps) {
            ...
            this.#callLifecycleHook(origin, id, handler).catch((error) => {
                logError(`Error calling lifecycle hook "${handler}" for Snap "${id}": ${getErrorMessage(error)}`);
            });
        }
    }
    /**
