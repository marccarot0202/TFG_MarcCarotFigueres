...
const concat = $importDefault($concat);
import $getNpmTarballUrl from "get-npm-tarball-url";
const getNpmTarballUrl = $importDefault($getNpmTarballUrl);
...
export class NpmLocation extends BaseNpmLocation {
    ...
    async fetchNpmTarball(tarballUrl) {
        ...
        assert(tarballResponse.status !== 404, `"${this.meta.packageName}" was not found in the NPM registry`);
        assert(tarballResponse.ok && tarballResponse.body, `Failed to fetch tarball for package "${this.meta.packageName}"`);
...
function isNPM(registryUrl) {
    ...
    return (registryUrlString === DEFAULT_NPM_REGISTRY.toString() ||
...
function createTarballStream(canonicalBase, files) {
    ...
    extractStream.on('entry', (header, entryStream, next) => {
        ...
        if (headerType === 'file') {
            ...
            return entryStream.pipe(concat({ encoding: 'uint8array' }, (data) => {
                try {
